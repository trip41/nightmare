...
Nightmare.prototype.registerEventHandlers = function (page, done) {
  ...
  phantomEvents.forEach(function(phantomEvent) {
    ...
    page.set(pageEvent, function() {
      ...
      // we change the emit type so we aren't emitting an actual error
      if(phantomEvent === 'error') {
        phantomEvent = 'consoleError';
      }
...
Nightmare.prototype.handleExit = function(code, signal) {
  ...
  if (code !== 0 && !this.pleaseDiePhantom) {
    var error = new Error('The phantomjs process ended unexpectedly: ' + code + ' - ' + signal);
    error.code = code;
    error.signal = signal;
...
Nightmare.prototype.afterNextPageLoad = function(done) {
  var self = this;
...
Nightmare.prototype.waitForElement = function(selector, opts, done) {
  debug('.wait() for the element ' + selector);
  var self = this;
  if(!opts) opts = {};

  var elementPresent = function(next) {
    self.page.evaluate(function (selector, opts) {
      try {
        var element;
...
Nightmare.prototype.waitForDomMutation = function(selector, prev, opts, done) {
  ...
  var moreMatchingElements = function(next) {
    page.evaluate(getDomDataForSelector, function (res) {
      ...
      else { debug('waiting for mutated elements', prev?prev.length:0, res.result?res.result.length:0); }
      prev = res.result;
      next({
        error: res.error,
        condition: condition
  ...
  function timedOut(res) {
    if(!res.condition) { self.emit('timeout', 'timeout elapsed before the dom mutated'); }
    done(res);
  }

  untilAsync(moreMatchingElements, self.options.pageLoadTimeout, self.options.interval, timedOut);
};
...
Nightmare.prototype.getDomState = function(selectors, opts, done) {
  var page = this.page;
  var domState = [];
  ...
  var runCheck = function() {
    ...
    check(function(res) {
      if (res.condition || diff > timeout) {
        ...
        checker = setTimeout(runCheck, interval);
      }
    });
  };
  runCheck();
...
function getDomDataForSelector(selector, opts) {
  if(!opts) { opts = {}; }

  try {
    var elements;
    if(opts.iframe && document.querySelector(opts.iframe)) {
      elements = Array.prototype.slice.call(document.querySelector(opts.iframe).contentDocument.querySelectorAll(selector));
    } else {
