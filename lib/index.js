...
Nightmare.prototype.waitForElement = function(selector, callback) {
  ...
  this.untilOnPage(elementPresent, true, function (present) {
    if (!present) self.emit('timeout', 'timeout elapsed before selector "' + selector + '" became present');
...
Nightmare.prototype.waitForDomMutation = function(selector, done) {
  ...
  until(hasCondition, this.options.timeout, this.options.interval, done);
};
