...
exports.goto = function(url, done) {
  ...
  // dont wait more than 10s for the page to load
  timeoutToken = setTimeout(function(){
    debug('.goto() timeout exceeded, moving on manually');
    movedOn = true;
    done();
  ...
  async.some(sels, function(item, done) {
    if (typeof item === 'number') {
      ...
      self.waitForDomMutation(item, state[i++], function(res) {
        ...
        if (!res) { self.emit('timeout', 'timeout elapsed before the dom mutated'); }
...
exports.wait = function(/* args */) {
  ...
  if (args.length === 1) {
    ...
    if (typeof condition === 'number') {
      ...
      selector = condition;
      debug('.wait() for an element');
      self.waitForElement(selector, done);
    }
    else if (condition instanceof Array) {
      selectors = condition;
      debug('.wait() for elements');
      //async.each(selectors, self.waitForElement.bind(self), done);
      async.some(selectors, self.waitForElement.bind(self), function(foundSome) {
        if(!foundSome) { self.emit('timeout', 'timeout elapsed before all selectors became present'); }
        done();
      });
    }
  }
    ...
    if(args[0] instanceof Array) {
      debug('.wait() for new elements or new page');
