...
/**
 * Include a JavaScript file onto the page
 *
 * @param {String} type
 * @param {String} file
 * @param {Function} done
...
exports.include = function(file, done){
  debug('.include()-ing js');
  this.page.includeJs(file, done);
};
...
exports.click = function(/* args */) {
  // click(selector)
  // click(selector, opts)
  // click(selector, opts, callback)
  // click(selector, filter, opts)
  // click(selector, filter, opts, callback)
  ...
  if(args.length === 3) {
    opts = args[1];
  } else if(args.length === 4 && typeof args[2] === 'function') {
    opts = args[1];
    cb = args[2];
  ...
  var self = this;
  self.page.evaluate(function (selector, filter, filterFnStr, opts) {
    var element;
    ...
    if(filter) {
      ...
      try {
        var elements;
        if(opts.iframe && document.querySelector(opts.iframe)) {
          elements = [].slice.call(document.querySelector(opts.iframe).contentDocument.querySelectorAll(selector));
        } else {
      ...
      try {
        if(opts.iframe && document.querySelector(opts.iframe)) {
          element = document.querySelector(opts.iframe).contentDocument.querySelector(selector);
        } else {
    ...
    if(element) {
      var event;
      if(opts.iframe && document.querySelector(opts.iframe)) {
        event = document.querySelector(opts.iframe).contentDocument.createEvent('MouseEvent');
      } else {
  ...
  }, selector, filter, filterFnStr, opts);
};
...
exports.type = function(/* args */) {
  ...
  //type(selector, text, opts, callback)
  ...
  var self = this;
  self.page.evaluate(function(selector, text, opts) {
    var element;
    try {
      if(opts.iframe) {
        element = document.querySelector(opts.iframe).contentDocument.querySelector(selector);
...
exports.select = function(/* args */) {
  //type(selector, val, opts)
  //type(selector, val, opts, callback)
  ...
  this.page.evaluate(function(selector, val, opts) {
    var element;
    try {
      if(opts.iframe) {
        element = document.querySelector(opts.iframe).contentDocument.querySelector(selector);
...
exports.domState = function(selectors, opts, cb, done) {
  this.getDomState(selectors, opts, function(err, state) {
    cb(err, state);
    done();
  });
};
...
exports.waitForNewElementsOrNewPage = function(/* args */) {
  // waitForNewElementsOrNewPage(selectors, state, opts, cb)
  ...
  async.some(sels, function(item, done) {
    if (typeof item === 'number') {
      ...
      self.waitForDomMutation(item, state[i++], opts, function(res) {
...
exports.waitForElements = function(selectors, opts, cb, done) {
  var self = this;
  var error;
  ...
  async.some(selectors, function(selector, next) {
    self.waitForElement(selector, opts, function(res) {
      error = res.error;
      next(res.condition);
    });
  }, function(res) {
...
exports.set = function(settings, done) {
  var self = this;
  async.each(Object.keys(settings), function(key, next) {
    var val = settings[key];
    debug('.set(\'settings.'+key+'\') to ' + val);
    self.page.set('settings.' + key, val, next);
  }, done);
};

/**
 * Prevent future resources from loading
 *
 * @param {String} key
 * @param {String} val
 * @param {Function} done
...
exports.preventResourceLoading = function(url, done) {
  this.page.onResourceRequested(
    function(requestData, request, url) {
      if (!(new RegExp('^'+url+'$')).test(requestData.url)) {
        request.abort();
      }
    },
    function(requestData, url) {
      if (!(new RegExp('^'+url+'$')).test(requestData.url)) {
        debug('Aborting resource request: ' + requestData.url);
      }
    },
    url
  );
