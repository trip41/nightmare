...
exports.click = function(/* args */) {
  ...
  var selector = args[0];
  var filter = (args.length === 3) ? args[1] : undefined;

  debug('.click() on ' + selector);
  this.page.evaluate(function (selector, filter) {
    var element;
    if(filter && window.kimono) {
      var elements = [].slice.call(document.querySelectorAll(selector));
      var filterFn = function(o) {
        return window.kimono.filters.regex(o.textContent, filter.regex);
      };
    ...
    if(element) {
      ...
      console.error('No matching element to click');
    }
  }, done, selector, filter);
};
...
/**
 * Fire done when first wait is finished
 *
 * @param {Null|Number|String|Function} condition
 */

exports.first = function(/* args */) {
  var self = this;
